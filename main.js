/* ============================================================
   Portfolio Main Script - Pacifique NIYISHOBORA
   Production Ready Version
   ============================================================
   
   EMAILJS CONFIGURATION:
   To receive emails at your Gmail address:
   
   1. Visit https://www.emailjs.com
   2. Create a free account
   3. Add Gmail as a service (connect your email)
   4. Create an email template with variables
   5. Copy your Public Key, Service ID, and Template ID
   6. Paste them in the variables below
...
themeButtons.forEach(btn => {
    ...
    if (theme) btn.addEventListener('click', () => setTheme(theme));
});
...
document.getElementById('applyGradient').addEventListener('click', () => {
    ...
    const grad = `linear-gradient(${currentGradDeg}deg, ${c1}, ${c2}, ${c3})`;
    document.querySelectorAll('.name-gradient').forEach(el => el.style.background = grad);
    document.querySelectorAll('.name-gradient').forEach(el => el.style.backgroundClip = 'text');
    document.querySelectorAll('.name-gradient').forEach(el => el.style.webkitBackgroundClip = 'text');
...
document.getElementById('resetGradient').addEventListener('click', () => {
    ...
    document.querySelectorAll('.name-gradient').forEach(el => el.style.background = 'linear-gradient(135deg, #60a5fa, #38bdf8)');
...
(function restoreSavedGradient() {
    ...
    try {
        ...
        updateGradientPreview();
        const grad = `linear-gradient(${deg}deg, ${c1}, ${c2}, ${c3})`;
        document.querySelectorAll('.name-gradient').forEach(el => el.style.background = grad);
    } catch (_) { }
})();
...
contactForm.addEventListener('submit', async function (e) {
    ...
    if (!isEmailJsReady()) {
        showFormStatus(
            '⚠ Email service is not configured yet. Please add your Public Key, Service ID, and Template ID in main.js.',
            'error'
        );
        return;
    }
...
if (downloadCVBtn) {
    downloadCVBtn.addEventListener('click', (e) => {
        e.preventDefault();
        // Replace 'YOUR_CV_FILE.pdf' with your actual CV file name
        const cvFile = 'cv.pdf'; // Make sure this file exists in your project
        const link = document.createElement('a');
        link.href = cvFile;
        link.download = 'Pacifique_NIYISHOBORA_CV.pdf';
        ...
        fetch(cvFile)
            .then(response => {
                if (response.ok) {
                    ...
                    alert('CV file not found. Please contact for CV.');
                }
            })
            .catch(() => {
                alert('CV file not available yet. Please contact for more information.');
            });
    });
}